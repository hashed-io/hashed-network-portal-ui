...
#container
  ...
  q-page-sticky(position="bottom-right" :offset="[18, 18]")
    q-btn(fab icon="refresh" color="secondary" @click="updateProposal")
      q-tooltip(self="bottom left" anchor="top left" :offset="[10, 10]") {{ $t('general.refresh') }}
  //- Header
  ...
  .row
    .col
      .text-subtitle2.q-mt-md {{ $t('pages.nbv.common.description') }}
      .text-body2 {{ description }}
    .col
      .text-subtitle2.q-mt-md {{ $t('pages.nbv.common.status') }}
      .text-body2 {{ labelStatus }}
  .row
    .col-xs-6.col-sm-6.col-md-4
      .text-subtitle2.q-mt-md {{ $t('pages.nbv.common.threshold') }}
      .text-body2 {{ threshold }}
    .col-xs-6.col-sm-6.col-md-4
      .text-subtitle2.q-mt-md {{ $t('pages.nbv.common.amountInSats') }}
      .text-body2 {{ amount }}
    .col-xs-12.col-sm-12.col-md-4
      .text-subtitle2.q-mt-md {{ $t('pages.nbv.proposals.feePerVB') }}
...
export default {
  ...
  computed: {
    ...
    labelActionBtn () {
      ...
      case 'Pending':
        if (this.canFinalize) {
          return this.$t('pages.nbv.proposals.finalizeTx')
        }
  ...
  methods: {
    ...
    handlerOffchainStatus (offchainStatus) {
      if (offchainStatus.IrrecoverableError) {
        ...
        this.offchainMessage = {
          message: this.$t('pages.nbv.proposals.pleaseAwaitOffchain'),
          status: 'loading'
        }
